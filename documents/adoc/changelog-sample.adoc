...
https://keepachangelog.com/en/1.1.0/[Keep a Changelog], and the project follows
...
* `Store.verify()` gained a `fast` parameter, hashing only the first and last
  chunk of each object. Roughly forty times quicker on large archives, and blind
  to corruption in the middle of an object.
...
* `list` sorts by path rather than by insertion order. Pass `--raw` for the old
  behaviour.
...
* `Store.get()` returns a stream rather than a `bytes` object.
+
IMPORTANT: Breaking for anyone who indexed the result, though the release is a
minor one because the method was documented as returning a file-like object
from the start.
...
* *Breaking.* The archive format changed. 2.x cannot read a 1.x archive, and 1.x
  cannot read a 2.x archive.
...
NOTE: 1.x is no longer maintained. Security fixes stopped on 2027-01-08.
